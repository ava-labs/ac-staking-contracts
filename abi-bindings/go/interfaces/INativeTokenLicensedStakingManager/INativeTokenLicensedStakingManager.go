...
// INativeTokenLicensedStakingManagerUnlockedIterator is returned from FilterUnlocked and is used to iterate over the raw logs and unpacked data for Unlocked events raised by the INativeTokenLicensedStakingManager contract.
type INativeTokenLicensedStakingManagerUnlockedIterator struct {
	Event *INativeTokenLicensedStakingManagerUnlocked // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *INativeTokenLicensedStakingManagerUnlockedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(INativeTokenLicensedStakingManagerUnlocked)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *INativeTokenLicensedStakingManagerUnlockedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *INativeTokenLicensedStakingManagerUnlockedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// INativeTokenLicensedStakingManagerUnlocked represents a Unlocked event raised by the INativeTokenLicensedStakingManager contract.
type INativeTokenLicensedStakingManagerUnlocked struct {
	To     common.Address
	Amount *big.Int
	Raw    types.Log // Blockchain specific contextual infos
}
...
func (_INativeTokenLicensedStakingManager *INativeTokenLicensedStakingManagerFilterer) FilterUnlocked(opts *bind.FilterOpts, to []common.Address) (*INativeTokenLicensedStakingManagerUnlockedIterator, error) {
	...
	logs, sub, err := _INativeTokenLicensedStakingManager.contract.FilterLogs(opts, "Unlocked", toRule)
	if err != nil {
		return nil, err
	}
	return &INativeTokenLicensedStakingManagerUnlockedIterator{contract: _INativeTokenLicensedStakingManager.contract, event: "Unlocked", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event Unlocked(address indexed to, uint256 amount)
func (_INativeTokenLicensedStakingManager *INativeTokenLicensedStakingManagerFilterer) WatchUnlocked(opts *bind.WatchOpts, sink chan<- *INativeTokenLicensedStakingManagerUnlocked, to []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _INativeTokenLicensedStakingManager.contract.WatchLogs(opts, "Unlocked", toRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(INativeTokenLicensedStakingManagerUnlocked)
				if err := _INativeTokenLicensedStakingManager.contract.UnpackLog(event, "Unlocked", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event Unlocked(address indexed to, uint256 amount)
func (_INativeTokenLicensedStakingManager *INativeTokenLicensedStakingManagerFilterer) ParseUnlocked(log types.Log) (*INativeTokenLicensedStakingManagerUnlocked, error) {
	event := new(INativeTokenLicensedStakingManagerUnlocked)
	if err := _INativeTokenLicensedStakingManager.contract.UnpackLog(event, "Unlocked", log); err != nil {
		return nil, err
	}
